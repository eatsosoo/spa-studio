...
type ProfileForm = CommonFormModel & { name: string; phone: string; email: string; gender: string; dateOfBirth: string; address: string; marketingConsent: boolean }
type PasswordForm = CommonFormModel & { currentPassword: string; newPassword: string; confirmPassword: string }
...
const { data: profileResponse, pending, error: profileLoadError, refresh } = await useAsyncData('customer-profile', () => customer.value ? $fetch<{ data: Profile }>('/api/customer/profile') : Promise.resolve(null), { watch: [customer] })
const profile = computed(() => profileResponse.value?.data)
const form = reactive<ProfileForm>({ name: '', phone: '', email: '', gender: '', dateOfBirth: '', address: '', marketingConsent: false })
...
watch(profile, (value) => {
  if (!value) return
  Object.assign(form, { name: value.name, phone: value.phone, email: value.email, gender: value.gender ?? '', dateOfBirth: value.dateOfBirth ?? '', address: value.address, marketingConsent: value.marketingConsent })
}, { immediate: true })

const profileFields = computed<CommonFormField[]>(() => [
  { name: 'name', label: 'Họ và tên', type: 'text', required: true, minLength: 2, attrs: { autocomplete: 'name' }, messages: { required: 'Vui lòng nhập họ và tên.', minLength: 'Họ tên cần có ít nhất 2 ký tự.' } },
  { name: 'phone', label: 'Số điện thoại', type: 'tel', disabled: true, hint: 'Số điện thoại dùng để đăng nhập.' },
  { name: 'email', label: 'Email', type: 'email', placeholder: 'ten@email.com', attrs: { autocomplete: 'email' } },
  { name: 'gender', label: 'Giới tính', type: 'select', placeholder: 'Không chia sẻ', options: [{ label: 'Nữ', value: 'female' }, { label: 'Nam', value: 'male' }, { label: 'Khác', value: 'other' }] },
  { name: 'dateOfBirth', label: 'Ngày sinh', type: 'date' },
  { name: 'address', label: 'Địa chỉ', type: 'text', placeholder: 'Địa chỉ liên hệ', columns: { sm: 2 }, attrs: { autocomplete: 'street-address' }, maxLength: 255 },
  { name: 'marketingConsent', label: 'Nhận thông tin về ưu đãi và nội dung chăm sóc phù hợp từ MIÊN.', type: 'checkbox', columns: { sm: 2 } },
])
...
const passwordFields = computed<CommonFormField[]>(() => [
  ...
  },
  {
    ...
    messages: { required: 'Vui lòng nhập mật khẩu mới.', minLength: 'Mật khẩu mới cần ít nhất 8 ký tự.', pattern: 'Mật khẩu mới cần gồm cả chữ và số.' },
  },
  {
    ...
    required: true,
    attrs: { autocomplete: 'new-password' },
    validate: (value, values) => value === values.newPassword || 'Mật khẩu xác nhận chưa khớp.',
...
async function saveProfile() {
  ...
  try {
    await $fetch('/api/customer/profile', { method: 'PATCH', body: { name: form.name, email: form.email, gender: form.gender, dateOfBirth: form.dateOfBirth, address: form.address, marketingConsent: form.marketingConsent } })
      ...
      <template v-if="customer">
        <header class="flex flex-wrap items-end justify-between gap-6 border-b border-[#78816f]/25 pb-8">
          ...
          <nav class="flex flex-wrap gap-3" aria-label="Khu vực tài khoản">
            <NuxtLink to="/lich-cua-toi" class="button-primary">Lịch của tôi</NuxtLink>
            <NuxtLink to="/don-hang" class="button-quiet">Đơn hàng</NuxtLink>
            <NuxtLink to="/danh-gia" class="button-quiet">Đánh giá</NuxtLink>
            <button type="button" class="button-quiet" @click="signOut">Đăng xuất</button>
          </nav>
        </header>

        <div v-if="pending" class="mt-8 space-y-4" aria-live="polite" aria-label="Đang tải hồ sơ">
          <div class="h-24 animate-pulse bg-[#e5dfd1]" />
          <div class="h-56 animate-pulse bg-[#e5dfd1]" />
        </div>
        <section v-else-if="profileLoadError" class="mt-8 border border-[#a96e64]/30 bg-[#f0dfda] p-6 text-[#78473f]" role="alert">
          <h2 class="text-lg font-semibold">Chưa thể tải hồ sơ.</h2>
          <p class="mt-2 text-sm">{{ errorText(profileLoadError) }}</p>
          <AppButton class="mt-5" label="Thử lại" variant="secondary" icon="refresh" @click="refresh" />
        </section>
        <template v-else-if="profile">
          ...
          <div class="mt-12 grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            >
              ...
              <template #messages>
                <div class="mt-3 min-h-6" aria-live="polite">
                  <p v-if="profileError" class="text-sm text-[#8b5148]" role="alert">{{ profileError }}</p>
                  <p v-else-if="profileMessage" class="text-sm text-[#56704f]" role="status">{{ profileMessage }}</p>
                </div>
              </template>
              <template #actions>
                <AppButton class="mt-3" :label="profileBusy ? 'Đang lưu…' : 'Lưu hồ sơ'" icon="check" type="submit" :disabled="profileBusy" />
              </template>
            </CommonForm>
            ...
            >
              ...
              <template #messages>
                <div class="mt-3 min-h-6" aria-live="polite">
                  <p v-if="passwordError" class="text-sm text-[#8b5148]" role="alert">{{ passwordError }}</p>
                  <p v-else-if="passwordMessage" class="text-sm text-[#56704f]" role="status">{{ passwordMessage }}</p>
                </div>
              </template>
              <template #actions>
                <AppButton class="mt-3" :label="passwordBusy ? 'Đang lưu…' : profile.hasPassword ? 'Đổi mật khẩu' : 'Tạo mật khẩu'" icon="lock" type="submit" :disabled="passwordBusy" />
              </template>
            </CommonForm>
          </div>
        </template>
