...
async function loadOrder() {
  const token = localStorage.getItem(`mien-order-${route.params.reference}`) ?? ''
  if (!token) { errorMessage.value = 'Không có quyền tra cứu đơn hàng trên thiết bị này.'; pending.value = false; return }
  try {
    const response = await $fetch<{ data: StoreOrder }>(`/api/orders/${route.params.reference}`, { headers: { Authorization: `Bearer ${token}` } })
