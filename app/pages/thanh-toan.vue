...
async function submitOrder() {
  ...
  try {
    ...
    clear()
    await router.push(`/don-hang/${response.data.reference}`)
  } catch (error) {
    errorMessage.value = failureMessage(error)
    await validateCart()
  } finally {
    ...
    <main class="px-5 pb-28 pt-12 md:px-10 md:pt-20 lg:px-14">
      ...
      <div v-else class="grid gap-14 py-12 lg:grid-cols-[minmax(0,1fr)_380px] lg:gap-24">
        <form id="checkout-form" class="grid gap-8" @submit.prevent="submitOrder">
          ...
          <fieldset class="grid gap-5 sm:grid-cols-2"><legend class="mb-5 text-sm font-semibold">Người nhận</legend><label class="field-block sm:col-span-2">Họ và tên<CommonInput v-model.trim="form.customerName" required minlength="2" autocomplete="name" /></label><label class="field-block">Số điện thoại<CommonInput v-model.trim="form.customerPhone" required type="tel" autocomplete="tel" placeholder="090 000 0000" /></label><label class="field-block">Email <i>Không bắt buộc</i><CommonInput v-model.trim="form.customerEmail" type="email" autocomplete="email" /></label></fieldset>
          <fieldset class="grid gap-5 sm:grid-cols-2"><legend class="mb-5 text-sm font-semibold">Địa chỉ giao hàng</legend><label class="field-block sm:col-span-2">Số nhà, tên đường<CommonInput v-model.trim="form.shippingAddressLine" required autocomplete="address-line1" /></label><label class="field-block">Phường / xã<CommonInput v-model.trim="form.shippingWard" autocomplete="address-level3" /></label><label class="field-block">Quận / huyện<CommonInput v-model.trim="form.shippingDistrict" required autocomplete="address-level2" /></label><label class="field-block sm:col-span-2">Tỉnh / thành phố<CommonInput v-model.trim="form.shippingProvince" required autocomplete="address-level1" /></label><label class="field-block sm:col-span-2">Ghi chú <i>Không bắt buộc</i><CommonTextarea v-model.trim="form.customerNote" rows="3" /></label></fieldset>
          <fieldset><legend class="mb-5 text-sm font-semibold">Thanh toán</legend><div class="grid gap-3"><label class="flex cursor-pointer gap-3 border border-[#78816f]/25 p-4 text-sm"><CommonInput v-model="form.paymentMethod" type="radio" value="cod" />Thanh toán khi nhận hàng</label><label class="flex cursor-pointer gap-3 border border-[#78816f]/25 p-4 text-sm"><CommonInput v-model="form.paymentMethod" type="radio" value="bank_transfer" />Chuyển khoản ngân hàng</label></div></fieldset>
        </form>
        <aside class="h-max border-t border-[#78816f]/25 pt-6 lg:sticky lg:top-28"><h2 class="text-sm font-semibold">Đơn hàng</h2><div class="mt-5 divide-y divide-[#78816f]/15 border-y border-[#78816f]/15"><div v-for="product in products" :key="product.id" class="flex justify-between gap-5 py-4 text-xs"><span>{{ product.name }} × {{ product.requestedQuantity }}<small v-if="product.message" class="mt-1 block text-[#8b5148]">{{ product.message }}</small></span><strong>{{ formatPrice(product.price * product.requestedQuantity) }}</strong></div></div><div class="mt-5 grid gap-3 text-xs"><div class="flex justify-between"><span>Tạm tính</span><span>{{ formatPrice(subtotal) }}</span></div><div class="flex justify-between"><span>Phí giao hàng</span><span>{{ shippingFee ? formatPrice(shippingFee) : 'Miễn phí' }}</span></div><div class="flex justify-between border-t border-[#78816f]/25 pt-4 text-base"><strong>Tổng cộng</strong><strong>{{ formatPrice(total) }}</strong></div></div><button form="checkout-form" type="submit" class="button-primary mt-7 w-full justify-center" :disabled="submitting || !validCart">{{ submitting ? 'Đang gửi đơn…' : 'Đặt hàng' }} <AppIcon v-if="!submitting" name="arrow" :size="16" /></button><NuxtLink v-if="!validCart" to="/gio-hang" class="text-link mt-4 block text-center">Quay lại giỏ hàng</NuxtLink><p class="mt-4 text-[0.66rem] leading-5 text-[#737a70]">MIÊN sẽ liên hệ xác nhận đơn. Số lượng được giữ sau khi nhân viên kiểm tra và xác nhận.</p></aside>
      </div>
    </div></main>
    <SiteFooter />
  </div>
