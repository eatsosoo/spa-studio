...
function normalizeBreakpoint(
  value: CommonFormBreakpointValue | undefined,
  fallback = 1,
) {
...
function resolveCondition(
  condition: CommonFormCondition | undefined,
  fallback = false,
...
function isVisible(field: CommonFormField, values = props.modelValue) {
  if (
    ...
    !resolveCondition(field.showWhen, true, values)
  )
    return false;
...
const visibleFields = computed(() =>
  props.fields.filter((field) => isVisible(field)),
);
const fieldId = (field: CommonFormField) =>
  `common-form-${instanceId}-${field.name}`;
const radioOptionId = (field: CommonFormField, index: number) => {
  const firstFocusableIndex =
    field.options?.findIndex((option) => !option.disabled) ?? 0;
  return index === Math.max(firstFocusableIndex, 0)
    ? fieldId(field)
    : `${fieldId(field)}-${index}`;
};
...
const fieldError = (field: CommonFormField) =>
  props.errors[field.name] || internalErrors.value[field.name] || "";

function customFieldBindings(
  field: CommonFormField,
): CommonFormCustomFieldBindings {
  return {
    id: fieldId(field),
    name: field.name,
...
function validateField(field: CommonFormField, values = props.modelValue) {
  ...
  if (
    ...
    (isBlank(value) || (field.type === "checkbox" && value !== true))
  ) {
    return (
      messages.required ??
      `Vui lòng nhập ${field.label?.toLocaleLowerCase("vi-VN") || "trường này"}.`
    );
  }
  ...
  if (field.min !== undefined) {
    const invalid =
      field.type === "number"
        ? Number.isNaN(numericValue) || numericValue < field.min
        : stringValue.length < field.min;
    if (invalid)
      return (
        messages.min ??
        (field.type === "number"
          ? `Giá trị nhỏ nhất là ${field.min}.`
          : `Vui lòng nhập ít nhất ${field.min} ký tự.`)
      );
  }
  if (field.max !== undefined) {
    const invalid =
      field.type === "number"
        ? Number.isNaN(numericValue) || numericValue > field.max
        : stringValue.length > field.max;
    if (invalid)
      return (
        messages.max ??
        (field.type === "number"
          ? `Giá trị lớn nhất là ${field.max}.`
          : `Vui lòng nhập không quá ${field.max} ký tự.`)
      );
  }
  if (field.minLength !== undefined && stringValue.length < field.minLength)
    return (
      messages.minLength ?? `Vui lòng nhập ít nhất ${field.minLength} ký tự.`
    );
  if (field.maxLength !== undefined && stringValue.length > field.maxLength)
    return (
      messages.maxLength ?? `Vui lòng nhập không quá ${field.maxLength} ký tự.`
    );
  ...
  const format =
    ...
    (field.type === "email"
      ...
      : field.type === "tel"
        ? "phone"
        : undefined);
  if (format === "email" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(stringValue))
    return messages.email ?? "Email chưa đúng định dạng.";
  if (format === "phone") {
    const normalizedPhone = stringValue
      .replace(/[\s.-]/g, "")
      .replace(/^\+84/, "0");
    if (!/^0\d{9}$/.test(normalizedPhone))
      return messages.phone ?? "Số điện thoại chưa đúng định dạng.";
  }
...
function validateTouchedFields(values: CommonFormModel) {
  ...
  for (const candidate of props.fields) {
    if (
      !touched.value.has(candidate.name) &&
      !internalErrors.value[candidate.name]
    )
      continue;
...
function updateField(field: CommonFormField, value: unknown) {
  ...
  if (props.validateOn === "change" || submitted.value) {
    ...
    Object.keys(internalErrors.value).length > 0
  ) {
...
function handleBlur(field: CommonFormField) {
  ...
  if (props.validateOn === "blur")
    setInternalError(field.name, validateField(field));
}
...
function handleSubmit() {
  ...
  if (!result.valid) {
    ...
    nextTick(() =>
      document
        .getElementById(
          fieldId(props.fields.find((field) => result.errors[field.name])!),
        )
        ?.focus(),
    );
    return;
...
function applyDefaults() {
  ...
  for (const field of props.fields) {
    if (
      (field.name in values && values[field.name] !== undefined) ||
...
<template>
  <form v-bind="$attrs" novalidate @submit.prevent="handleSubmit">
    <slot />
    <div
      class="common-form__grid"
      :class="{ 'common-form__grid--after-content': Boolean($slots.default) }"
      :style="gridStyle"
    >
      <div
      ...
      >
        ...
        >
          ...
          <div
            class="common-form__choices"
            :class="
              field.choiceLayout === 'row' ? 'common-form__choices--row' : ''
            "
          >
            <label
              v-for="(option, optionIndex) in field.options ?? []"
              :key="`${String(option.value)}-${optionIndex}`"
              class="common-form__choice-label"
              :for="radioOptionId(field, optionIndex)"
            >
              <CommonInput
                ...
                "
                :disabled="
                  isDisabled(field) || isReadonly(field) || option.disabled
                "
                :aria-readonly="isReadonly(field) || undefined"
                :aria-required="isRequired(field) || undefined"
                :aria-invalid="Boolean(fieldError(field))"
        ...
        <template v-else>
          ...
          >
            <CommonSelect
              ...
              :id="fieldId(field)"
              :model-value="
                fieldValue(field) as string | number | null | undefined
              "
              :disabled="isDisabled(field) || isReadonly(field)"
              :required="isRequired(field)"
              :aria-readonly="isReadonly(field) || undefined"
        ...
        <div
          :id="messageId(field)"
          class="common-form__message"
          aria-live="polite"
        >
          <span v-if="fieldError(field)" class="common-form__error">{{
            fieldError(field)
          }}</span>
          <span v-else-if="field.hint" class="common-form__hint">{{
            field.hint
          }}</span>
...
.common-form__grid--after-content {
  margin-top: 1.75rem;
}
...
.common-form__field
  :deep(.common-field:not([type="checkbox"]):not([type="radio"])) {
  width: 100%;
...
.common-form__field :deep(textarea.common-field) {
  resize: vertical;
}

.common-form__field
  :deep(.common-field:not([type="checkbox"]):not([type="radio"]):focus) {
  border-color: #596650;
  background: #fbf8f0;
  outline: none;
}

.common-form__field :deep(.common-field[aria-invalid="true"]) {
  border-color: #9b6056;
}
.common-form__field :deep(.common-field:disabled) {
  cursor: not-allowed;
  opacity: 0.62;
}
...
@media (min-width: 640px) {
  .common-form__grid {
    grid-template-columns: repeat(
      var(--common-form-columns-sm),
      minmax(0, 1fr)
    );
  }
  .common-form__field {
    grid-column: span var(--common-form-span-sm);
  }
}
...
@media (min-width: 768px) {
  .common-form__grid {
    grid-template-columns: repeat(
      var(--common-form-columns-md),
      minmax(0, 1fr)
    );
  }
  .common-form__field {
    grid-column: span var(--common-form-span-md);
  }
}
...
@media (min-width: 1024px) {
  .common-form__grid {
    grid-template-columns: repeat(
      var(--common-form-columns-lg),
      minmax(0, 1fr)
    );
  }
  .common-form__field {
    grid-column: span var(--common-form-span-lg);
  }
}
</style>
