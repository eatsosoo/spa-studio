        ...
        <aside class="absolute right-0 top-0 h-full w-full max-w-[680px] overflow-y-auto bg-[#f6f3eb] px-5 py-7 shadow-[-24px_0_70px_rgba(38,45,34,0.14)] md:px-9 md:py-9">
          ...
          <form class="mt-7 grid gap-6" novalidate @submit.prevent="submit(true)">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <label class="admin-field"><span>Loại chứng từ</span><CommonSelect v-model="type"><option value="receipt">Nhập kho</option><option value="adjustment">Điều chỉnh kiểm kê</option><option value="transfer">Điều chuyển kho</option><option value="return">Khách trả hàng</option></CommonSelect></label>
              <label class="admin-field"><span>Thời điểm</span><CommonInput v-model="occurredAt" type="datetime-local" :aria-invalid="Boolean(errors.occurredAt)" /><small v-if="errors.occurredAt" class="text-[#8b5148]">{{ errors.occurredAt }}</small></label>
              <label v-if="type === 'transfer'" class="admin-field"><span>Kho nguồn</span><CommonSelect v-model.number="sourceLocationId" :aria-invalid="Boolean(errors.sourceLocationId)"><option value="" disabled>Chọn kho nguồn</option><option v-for="location in locations" :key="location.id" :value="location.id">{{ location.name }} · {{ location.code }}</option></CommonSelect><small v-if="errors.sourceLocationId" class="text-[#8b5148]">{{ errors.sourceLocationId }}</small></label>
              <label class="admin-field"><span>{{ type === 'transfer' ? 'Kho nhận' : 'Kho' }}</span><CommonSelect v-model.number="destinationLocationId" :aria-invalid="Boolean(errors.destinationLocationId)"><option value="" disabled>Chọn kho</option><option v-for="location in locations" :key="location.id" :value="location.id">{{ location.name }} · {{ location.code }}</option></CommonSelect><small v-if="errors.destinationLocationId" class="text-[#8b5148]">{{ errors.destinationLocationId }}</small></label>
              <label v-if="type === 'receipt'" class="admin-field"><span>Nhà cung cấp</span><CommonInput v-model="supplierName" placeholder="Tên nhà cung cấp" /></label>
              <label v-if="type === 'receipt'" class="admin-field"><span>Số hóa đơn</span><CommonInput v-model="invoiceNumber" placeholder="Số hóa đơn hoặc phiếu giao" /></label>
              <label v-if="type === 'return'" class="admin-field sm:col-span-2"><span>Đơn bán gốc</span><CommonSelect v-model.number="sourceOrderId" :aria-invalid="Boolean(errors.sourceOrderId)"><option value="" disabled>Chọn đơn đã thanh toán</option><option v-for="order in orders" :key="order.id" :value="order.id">{{ order.reference }}</option></CommonSelect><small v-if="errors.sourceOrderId" class="text-[#8b5148]">{{ errors.sourceOrderId }}</small><small v-else>Hệ thống kiểm tra số lượng trả không vượt quá số đã bán.</small></label>
            </div>
            ...
            <section class="border-t border-[#78816f]/20 pt-6">
              ...
              <div class="mt-4 grid gap-4">
                <div v-for="(item, index) in items" :key="index" class="relative grid gap-4 border-t border-[#78816f]/15 pt-5 first:border-0 first:pt-0 sm:grid-cols-2">
                  <label class="admin-field sm:col-span-2"><span>Sản phẩm {{ index + 1 }}</span><CommonSelect v-model.number="item.productId" :aria-invalid="Boolean(errors[`product-${index}`])"><option value="" disabled>Chọn sản phẩm</option><option v-for="product in products" :key="product.id" :value="product.id">{{ product.name }} · {{ product.sku }}</option></CommonSelect><small v-if="errors[`product-${index}`]" class="text-[#8b5148]">{{ errors[`product-${index}`] }}</small></label>
                  <label class="admin-field"><span>Số lượng</span><CommonInput v-model.number="item.quantity" type="number" min="0.001" step="0.001" :aria-invalid="Boolean(errors[`quantity-${index}`])" /><small v-if="errors[`quantity-${index}`]" class="text-[#8b5148]">{{ errors[`quantity-${index}`] }}</small></label>
                  <label v-if="type === 'receipt'" class="admin-field"><span>Đơn giá nhập</span><CommonInput v-model.number="item.unitCost" type="number" min="0" step="1" placeholder="Không bắt buộc" /><small v-if="errors[`cost-${index}`]" class="text-[#8b5148]">{{ errors[`cost-${index}`] }}</small></label>
                  <label v-if="type === 'adjustment'" class="admin-field"><span>Chiều điều chỉnh</span><CommonSelect v-model="item.direction"><option value="increase">Tăng tồn</option><option value="decrease">Giảm tồn</option></CommonSelect></label>
                  <label v-if="type === 'adjustment'" class="admin-field"><span>Lý do</span><CommonInput v-model="item.reasonCode" placeholder="Kiểm kê lệch, hỏng, hết hạn…" :aria-invalid="Boolean(errors[`reason-${index}`])" /><small v-if="errors[`reason-${index}`]" class="text-[#8b5148]">{{ errors[`reason-${index}`] }}</small></label>
                  <label v-if="type === 'receipt'" class="admin-field"><span>Số lô</span><CommonInput v-model="item.batchNumber" placeholder="Không bắt buộc" /></label>
                  <label v-if="type === 'receipt'" class="admin-field"><span>Hạn sử dụng</span><CommonDatePicker v-model="item.expiryDate" placeholder="Chọn hạn sử dụng" /></label>
                  <label v-if="type === 'return'" class="admin-field sm:col-span-2"><span>Tình trạng hàng trả</span><CommonSelect v-model="item.disposition"><option value="sellable">Còn nguyên vẹn · nhập lại tồn bán</option><option value="damaged">Hư hỏng · chỉ ghi nhận, không nhập tồn</option></CommonSelect></label>
            ...
            <label class="admin-field"><span>Ghi chú chứng từ</span><CommonTextarea v-model="note" rows="3" placeholder="Thông tin cần lưu để đối soát" /></label>
