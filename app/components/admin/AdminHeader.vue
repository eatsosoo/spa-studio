...
const { user, logoutPending, logout, can } = useAdminAuth()
        ...
        <Transition name="fade">
          <div v-if="menuOpen" class="absolute right-0 top-[calc(100%+0.65rem)] w-64 overflow-hidden rounded-md border border-[#78816f]/20 bg-[#fbf8f0] shadow-[0_18px_50px_rgba(49,58,45,0.13)]" role="menu">
            ...
            <div class="p-2">
              ...
              <NuxtLink v-if="can('settings.read')" to="/admin/cau-hinh" class="flex items-center gap-3 rounded-sm px-3 py-2.5 text-xs text-[#4d5848] transition hover:bg-[#e9e5da]" role="menuitem"><AppIcon name="settings" :size="17" />Cấu hình hệ thống</NuxtLink>
