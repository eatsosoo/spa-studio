...
import { employees, postCategories, productCategories, services } from '../../database/schema'
import { useDatabase } from '../../database/client'

export default defineEventHandler(async () => {
  const db = useDatabase()
  const [serviceRows, employeeRows, productCategoryRows, postCategoryRows] = await Promise.all([
  ...
  return { data: { services: serviceRows.map(item => item.name), employees: employeeRows.map(item => item.name), productCategories: productCategoryRows.map(item => item.name), postCategories: postCategoryRows.map(item => item.name) } }
})
