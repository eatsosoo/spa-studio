...
export default defineEventHandler(async (event) => {
  ...
  const urls = [...['/san-pham', '/kinh-nghiem'].map(path => `<url><loc>${escapeXml(siteUrl + path)}</loc></url>`), ...productRows.map(row => `<url><loc>${escapeXml(siteUrl + '/san-pham/' + row.slug)}</loc><lastmod>${new Date(row.updatedAt).toISOString()}</lastmod></url>`), `<url><loc>${escapeXml(siteUrl)}</loc></url>`, `<url><loc>${escapeXml(`${siteUrl}/bai-viet`)}</loc></url>`, ...rows.map(row => `<url><loc>${escapeXml(`${siteUrl}/bai-viet/${row.slug}`)}</loc><lastmod>${new Date(row.updatedAt).toISOString()}</lastmod></url>`)]
